...
const FormularioAgendamento: React.FC<{ clienteId: number }> = ({ clienteId }) => {
  ...
  return (
    <div className="flex flex-col w-full max-w-2xl p-6 bg-white rounded-2xl shadow-lg border">
      {passoAtual === 0 && (
        <div>
          ...
          {funcionarios.map((func) => (
            <button
              key={func.id}
              className={`p-2 border rounded-lg w-full my-2 ${detalhesAgendamento.funcionarioId === func.id ? "bg-blue-500 text-white" : ""}`}
              onClick={() => setDetalhesAgendamento({ ...detalhesAgendamento, funcionarioId: func.id, funcionarioNome: func.nome })}
            >
              {func.nome}
            </button>
          ))}
      ...
      {passoAtual === 1 && (
        <div>
          ...
          {servicos.map((servico) => (
            <button
              key={servico.id}
              className={`p-2 border rounded-lg w-full my-2 ${detalhesAgendamento.servicoId === servico.id ? "bg-blue-500 text-white" : ""}`}
              onClick={() => setDetalhesAgendamento({ ...detalhesAgendamento, servicoId: servico.id, servicoNome: servico.nome })}
            >
      ...
      {passoAtual === 2 && (
        <div>
          <h2 className="text-xl font-semibold">Escolha a Data e Hora</h2>
          <input type="date" className="w-full p-2 border rounded-lg" value={detalhesAgendamento.data} onChange={(e) => setDetalhesAgendamento({ ...detalhesAgendamento, data: e.target.value })} />
          <input type="time" className="w-full p-2 border rounded-lg mt-2" value={detalhesAgendamento.horario} onChange={(e) => setDetalhesAgendamento({ ...detalhesAgendamento, horario: e.target.value })} />
        </div>
      )}
      ...
      {passoAtual === 3 && (
        <div>
          ...
          <textarea className="w-full p-2 border rounded-lg" value={detalhesAgendamento.descricao} onChange={(e) => setDetalhesAgendamento({ ...detalhesAgendamento, descricao: e.target.value })} />
        </div>
      )}

      <div className="flex justify-between border-t pt-4">
        {passoAtual > 0 && (
          <button className="px-4 py-2 bg-gray-400 text-white rounded-lg" onClick={voltarPasso}>Voltar</button>
        )}
        {passoAtual < 3 ? (
          <button className="px-4 py-2 bg-blue-500 text-white rounded-lg" onClick={avancarPasso}>Próximo</button>
        ) : (
          <button className="px-4 py-2 bg-green-500 text-white rounded-lg" onClick={() => setMostrarModal(true)}>Confirmar</button>
        )}
      </div>
