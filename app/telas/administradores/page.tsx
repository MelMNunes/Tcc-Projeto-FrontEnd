...
const ServicesIcon = () => (
  ...
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M21.75 9v.906a2.25 2.25 0 0 1-1.183 1.981l-6.478 3.488M2.25 9v.906a2.25 2.25 0 0 0 1.183 1.981l6.478 3.488m8.839 2.51-4.66-2.51m0 0-1.023-.55a2.25 2.25 0 0 0-2.134 0l-1.023.55m0 0-4.661 2.51m16.5 1.615a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V8.844a2.25 2.25 0 0 1 1.183-1.982l7.5-4.04a2.25 2.25 0 0 1 2.134 0l7.5 4.04a2.25 2.25 0 0 1 1.183 1.982V17.25Z"
    />
  </svg>
);
const ProfileIcon = () => (
...
const AdminPage = () => {
  ...
  const fetchAdminDataFromStorage = useCallback(() => {
    ...
    if (userDataString) {
      try {
        ...
        console.log(
          "AdminPage: Dados brutos do localStorage ('user'):",
          userData
        );
        console.log(
          "AdminPage: Valor de userData.cpf:",
          userData.cpf,
          "| Tipo:",
          typeof userData.cpf
        );
        console.log(
          "AdminPage: Valor de userData.telefone:",
          userData.telefone,
          "| Tipo:",
          typeof userData.telefone
        );
        ...
        console.error(
          "Erro ao recuperar dados do admin do localStorage:",
          error
        );
      ...
      alert(
        "Sessão não encontrada ou dados de usuário ausentes no localStorage. Redirecionando..."
      );
      setLoading(false);
      router.push("/");
    }
  ...
  const fetchServicos = useCallback(async () => {
    ...
    try {
      ...
      alert(
        error instanceof Error ? error.message : "Falha ao carregar serviços."
      );
      setServicos([]);
    } finally {
      setLoading(false);
  ...
  const handleAddServico = async (e: React.FormEvent) => {
    ...
    if (isNaN(duracaoValida) || duracaoValida <= 0) {
      alert(
        "Por favor, insira uma duração válida (número positivo em minutos)."
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Erro ao adicionar serviço: ${errorData || response.status}`
        );
      }
      setNewServico({
        nome: "",
  ...
  const handleDeleteServico = async (id?: number) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(`Erro ${response.status} ao excluir serviço.`);
      fetchServicos();
      ...
      alert(
        error instanceof Error
          ? error.message
          : "Ocorreu um erro ao excluir o serviço."
      );
    }
  };
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNewTelefone(
          processField(newTelefone) === "Não informado"
            ? ""
            : processField(newTelefone)
        );
        setNewSenha("");
      ...
      alert(
        `Ocorreu um erro de rede ou inesperado: ${
        ...
        return (
          <section className="bg-white p-4 sm:p-6 rounded-xl shadow-lg">
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
              ...
              >
                <UsersIcon /> <span className="ml-2">Cadastrar Novo Usuário</span>
              </button>
            </div>
            <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
              <div>
                ...
                <input
                  ...
                  className="w-full text-black rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 p-3 text-sm"
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                />
              </div>
              <div>
                <label htmlFor="filterUserType" className="sr-only">
                  Filtrar por tipo
                </label>
                <select
                  id="filterUserType"
                      ...
                      >
                          ...
                          <span
                            className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${
                              user.tipoDeUsuario === "ADMINISTRADOR"
                                ...
                                : user.tipoDeUsuario === "FUNCIONARIO"
                                ? "bg-yellow-100 text-yellow-800"
                                : user.tipoDeUsuario === "CLIENTE"
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <button
                            ...
                            className="text-red-600 hover:text-red-800 transition duration-150 hover:underline"
        ...
        return (
          <section className="bg-white p-4 sm:p-6 rounded-xl shadow-lg text-black">
            ...
            <form
              ...
              className="mb-8 p-4 sm:p-6 border border-gray-200 rounded-lg shadow-sm bg-gray-50 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4"
            >
              <h3 className="text-xl font-medium text-gray-700 col-span-1 md:col-span-2 mb-2">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewServico({ ...newServico, nome: e.target.value })
                  }
              ...
              <div>
                ...
                <input
                  id="servicoDescricao"
                  type="text"
                  placeholder="Ex: Desencrava unhas"
                  value={newServico.descricao}
                  onChange={(e) =>
                    setNewServico({ ...newServico, descricao: e.target.value })
                  }
                  className="w-full border-gray-300 rounded-lg shadow-sm p-3 focus:ring-indigo-500 focus:border-indigo-500 text-sm"
                  required
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewServico({ ...newServico, preco: e.target.value })
                  }
                  className="w-full border-gray-300 rounded-lg shadow-sm p-3 focus:ring-indigo-500 focus:border-indigo-500 text-sm"
                  min="0.01"
                  step="0.01"
              ...
              <div className="md:col-span-2 flex justify-end mt-2">
                <button
                  ...
                  className="bg-green-600 hover:bg-green-700 text-white font-medium py-2.5 px-6 rounded-lg shadow-md hover:shadow-lg transition duration-150 ease-in-out disabled:opacity-60 disabled:cursor-not-allowed w-full sm:w-auto"
                >
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {servicos.map((servico) => (
                      ...
                      >
                        ...
                        <td
                          className="px-6 py-4 whitespace-normal text-sm text-gray-600 max-w-xs truncate hidden md:table-cell"
            ...
            {!editingProfile ? (
              ...
              >
                <div>
                  ...
                  <p
                    id="profileNome"
                    className="text-sm p-3 bg-gray-100 rounded-md text-gray-500"
                  >
                    {adminData.nome}
                  </p>
                </div>
                <div>
                  ...
                  <p
                    id="profileCPF"
                    className="text-md p-3 bg-gray-100 rounded-md text-gray-500"
                  >
                    {adminData.cpf}
                  </p>
                </div>
                ...
                <div>
                  <label
                    htmlFor="profileSenha"
                    className="block text-sm font-medium text-gray-700 mb-1"
                  >
                    Nova Senha (mín. 8 caracteres, 1 letra maiuscula, 1 letra
                ...
                <div className="flex flex-col sm:flex-row gap-4 pt-2">
                  <button
                    type="submit"
                    className="w-full sm:w-auto flex-1 bg-green-600 hover:bg-green-700 text-white font-medium py-3 px-6 rounded-lg shadow-md hover:shadow-lg transition duration-150 ease-in-out"
                  >
                    Salvar Alterações
                  </button>
                  <button
    ...
    <div className="flex h-screen bg-gray-100 font-sans">
      <nav className="w-64 bg-white p-5 shadow-xl h-full fixed top-0 left-0 flex flex-col justify-between print:hidden transition-transform duration-300 ease-in-out">
        ...
        <button
          ...
          className="w-full flex items-center justify-center px-4 py-3 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg shadow-md hover:shadow-lg transition duration-150 ease-in-out"
        >
          <LogoutIcon />
        ...
        <header className="w-full bg-white shadow-md p-5 print:hidden sticky top-0 z-20 border-b border-gray-200">
          <div className="flex justify-between items-center">
            <h2 className="text-xl sm:text-2xl font-semibold text-gray-800 capitalize">
              {selectedTab
                .replace("usuarios", "Gerenciamento de Usuários")
                .replace("servicos", "Gerenciamento de Serviços")
                .replace("perfil", "Configurações de Perfil")
                .replace("recepcao", "Modo Recepção")}
            </h2>
            {adminData && (
